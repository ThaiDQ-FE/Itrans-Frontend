...
function FormInformationAboutTheOrganization(props) {
  ...
  const handleChange = (value, action) => {
    if (!action.length) {
      setInformation({
        ...information, [action.name]: value
      })
    } else {
      setInformation({
        ...information, [action[0].name]: value
      })
    }
  };
  const renderListProvince = () => {
    return listProvince.map((item, index) => {
      return <Option name="province" value={item.idProvince} key={index}>{item.name}</Option>
    })
  }
  const renderListStage = () => {
    return listStage.map((item, index) => {
      return <Option name="stage" value={item.idStage} key={index} >{item.name}</Option>
    })
  }
  const renderListIndustry = () => {
    return listIndustry.map((item, index) => {
      return <Option name="industry" value={item.idIndustry} key={index} disabled={item.active === false}>{item.name}</Option>
    })
  }
  for (let i = 10; i < 36; i++) {
    children.push(
        ...
        <form className="fiato__form">
          <div className="fiato__lineOne">
            <div className="fiato__tenToChuc">
              <Input name="name" onChange={handleChangeInput} placeholder="Tên tổ chức" size="large" />
            </div>
          ...
          <div className="fiato__lineTwo">
            <div className="fiato__giaiDoanPhatTrien">
              <Select onChange={handleChange} name="stage" placeholder="Giai đoạn phát triển" size="large">
                {renderListStage()}
              </Select>
            </div>
            <div className="fiato__namThanhLap">
              <Input name="foundedYear" onChange={handleChangeInput} placeholder="Năm thành lập" type="text" size="large" />
            </div>
            <div className="fiato__soLuongThanhVien">
              <Input name="numberOfEmployee" onChange={handleChangeInput} placeholder="Số lượng thành viên" size="large" />
            </div>
          </div>
          <div className="fiato__lineThree">
            ...
            <div className="fiato__linkWebsite">
              <Input name="link" onChange={handleChangeInput} placeholder="Link Website" size="large" />
            </div>
          </div>
